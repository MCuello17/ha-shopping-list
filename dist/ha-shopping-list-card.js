...
`,vt=[{name:"entity",required:!0,selector:{entity:{domain:"todo"}}},{name:"title",selector:{text:{}}},{name:"icon",selector:{icon:{}}},{name:"show_header",selector:{boolean:{}}},{name:"show_completed",selector:{boolean:{}}},{name:"show_add_input",selector:{boolean:{}}},{name:"add_button_label",selector:{text:{}}},{name:"empty_message",selector:{text:{}}},{name:"sort",selector:{select:{mode:"dropdown",options:[{value:"manual",label:"Manual (HA order)"},{value:"alpha",label:"Alphabetical"},{value:"created",label:"Created order"}]}}}];let yt=class extends at{constructor(){super(...arguments),this._labelFor=t=>({entity:"Todo Entity (required)",title:"Title",icon:"Icon",show_header:"Show header",show_completed:"Show completed items",show_add_input:"Show add-item input",add_button_label:"Add button label",empty_message:"Empty list message",sort:"Sort order"}[t.name]??t.name)}setConfig(t){this._config=t}render(){return this.hass&&this._config?W`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${vt}
    ...
    `:W``}_formValueChanged(t){if(t.stopPropagation(),!this._config)return;const e=t.detail.value,s={...this._config,...e};this._fireChange(s)}_styleChanged(t){if(t.stopPropagation(),!this._config)return;const e=t.detail?.value??"",s={...this._config,style:e};e||delete s.style,this._fireChange(s)}_fireChange(t){this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}};yt.styles=n`
    :host {
      display: block;
    }
    .group {
